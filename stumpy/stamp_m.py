...
def _mass_PI(
    ...
    Σ_T,
    T_subseq_isconstant,
    m,  #### CHANGE HERE restrict to multiples of window
    trivial_idx=None,
    excl_zone=0,
    left=False,
    right=False,
    ...
    if trivial_idx % m == 0:
        ...
        if trivial_idx is not None:
            ...
            if left:
                IL = -1
                PL = np.inf
                if D[:trivial_idx].size > m:
                    IL = np.argmin(D[:trivial_idx:m]) * m #### CHANGE HERE restrict to multiples of window
                    PL = D[IL]
                if PL == np.inf:
                    IL = -1
            ...
            if right:
                IR = -1
                PR = np.inf
                if D[trivial_idx + 1 :].size > m:
                    IR = ((trivial_idx + 1)//m + 1)*m + np.argmin(D[((trivial_idx + 1)//m + 1)*m::m]) * m #### CHANGE HERE restrict to multiples of window
                    PR = D[IR]
                if PR == np.inf:
                    IR = -1

        # Element-wise Min
        I = np.argmin(D[0::m]) * m  #### CHANGE HERE restrict to multiples of window
        P = D[I]
        if P == np.inf:
            I = -1
...
def stamp_m(T_A, T_B, m, ignore_trivial=False):
    """
    ...
    if ignore_trivial:
        out = [
            _mass_PI(subseq, T_B, M_T, Σ_T, T_subseq_isconstant, m, i, excl_zone) #### CHANGE HERE restrict to multiples of window
            for i, subseq in enumerate(subseq_T_A)
        ]
    else:
        out = [
            _mass_PI(subseq, T_B, M_T, Σ_T, T_subseq_isconstant, m) #### CHANGE HERE restrict to multiples of window
            for subseq in subseq_T_A
        ]
    out = np.array(out, dtype=object)

    out = out[0::m,:]
    out = np.repeat(out,m, axis=0)
